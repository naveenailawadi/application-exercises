...
- n_missing shows number of null values
...
```{r resort-children}
# create one group of hotels and another of 
hotels %>% 
...
do it combined!!!
```{r resort-city-children}
hotels %>% 
  filter(children >= 1 | babies >= 1) %>% 
  group_by(hotel) %>% 
  summarise(reservation_ct = n())
```

#### Distinct & Arrange
find all the distinct market segments (alphabetically with arrange)
```{r distinct-market-segment}
hotels %>% 
...
arrange by the type of hotel
```{r distinct-hotel-market-segment}
hotels %>% 
  distinct(hotel, market_segment) %>% 
  arrange(hotel, market_segment)
```

#### Counting Market Segments
sort in descending order too!
```{r count-market-segments}
hotels %>%  
...
**Note: order matters with arrange! (will arrange in order)
...
mutate it without any grouping
```{r hotel-mutants}
hotels %>% 
  mutate(
...
grouping by hotel can get more specific mutated data sorted by some type
```{r hotel-mutants-by-group}
hotels %>% 
  group_by(hotel) %>% 
  mutate(
    little_ones = children + babies) %>%
...
hotels %>%
  group_by(hotel) %>% 
  summarize(mean_adults = mean(adults))
```
