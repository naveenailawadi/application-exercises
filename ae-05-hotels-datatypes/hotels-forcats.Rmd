...
hotels = hotels %>%
  mutate(custom_date_order = factor(arrival_date_month, 
                              levels = c("January", "February", "March", "April", "June",
                                         "July", "August", "September", "October", 
                                         "November", "December")))

hotels %>%
  group_by(hotel, custom_date_order) %>%   # group by hotel type and arrival month
  summarise(mean_adr = mean(adr)) %>%       # calculate mean adr for each group
  ggplot(aes(
    x = custom_date_order,                 # x-axis = arrival_date_month (in alphabetical order due to factor status)
...
Create a new field by putting together strings (but keep the date objects)
```{r count-dates}
hotels = hotels %>% 
